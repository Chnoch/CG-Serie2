...
/**
 * Manages OpenGL shaders. This class will be used in 
 * the "Textures and Shading" project.
 */
public class GLShader implements Shader {
	...
	private String getCompilerOutputShader(int shaderObject) {
		...
		gl.glGetShaderiv(shaderObject, GL2.GL_INFO_LOG_LENGTH, ib);
	...
	private String getLinkerOutput(int shaderObject) {
		...
		gl.glGetProgramiv(shaderObject, GL2.GL_INFO_LOG_LENGTH, ib);
		int logLenght = ib.get(0);
		if(logLenght <= 2)
	...
	{	
		...
		gl.glGetShaderiv(v, GL2.GL_COMPILE_STATUS, status, 0);
		if(status[0] == GL.GL_FALSE) {
			throw new Exception("Could not compile vertex shader.");
		}
		gl.glGetShaderiv(f, GL2.GL_COMPILE_STATUS, status, 0);
		if(status[0] == GL.GL_FALSE) {
		...
		IntBuffer ib = ByteBuffer.allocateDirect(4).order(ByteOrder.nativeOrder()).asIntBuffer();
		ib.rewind();
		gl.glGetProgramiv(p, GL2.GL_LINK_STATUS, ib);
